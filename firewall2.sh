...
iptables -t nat -A PREROUTING -i enp0s8 -p tcp --dport 8080 -j DNAT --to 192.168.30.2:80
iptables -t nat -A PREROUTING -i enp0s8 -p tcp --dport 8443 -j DNAT --to 192.168.30.2:443
iptables -t nat -A PREROUTING -i enp0s8 -p tcp --dport 31 -j DNAT --to 192.168.32.1:31

# Permitir tráfico hacia debian5 por el puerto 31 (SSH) y hacia debian2 por los puertos 80 (HTTP) y 443 (HTTPS)
iptables -A FORWARD -d 192.168.32.1 -p tcp --dport 31 -j ACCEPT
